...
httpInstance.interceptors.request.use(
  async (
    ...
    // 加密处理标记
    const isNoEncryptUrl = configJson.noEncryptUrls.some((path) =>
      url?.includes(path)
    );
    ...
    if (!isNoEncryptUrl) {
      try {
        ...
        if (method?.toUpperCase() === 'GET') {
          ...
          const encryptedParams = config.params
            ? SafetyUtils.sm4Encrypt(sm4Key, config.params)
            : ''; // 无参数时encryptedData可为空
        ...
        if (
          ...
          if (config.data instanceof FormData) {
            // 加密表单数据
            const encryptedFormData = SafetyUtils.encryptFormData(
              sm4Key,
              config.data
            );
            ...
            const encryptedData = config.data
              ? SafetyUtils.sm4Encrypt(sm4Key, config.data)
              : '';
...
httpInstance.interceptors.response.use(
    ...
    if (config.isNoEncryptUrl || !config.__sm4Key) {
      ...
      try {
        if (typeof data === 'string') {
          ...
          // 解密对象中包含的加密字段
          processedData = SafetyUtils.sm4Decrypt(
            config.__sm4Key,
  ...
  (error) => {
    ...
    if (
      ...
      error.response?.data?.msg?.includes('解密')
    ) {
